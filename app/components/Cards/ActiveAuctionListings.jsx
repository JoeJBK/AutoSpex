...
import { Card, CardHeader, CardBody, CardFooter, Image, Link, Button, Spinner } from "@nextui-org/react";
...
export default function ActiveDealerListings({
  ...
  useEffect(() => {
    const fetchData = async () => {
      setLoading(true);
      try {
        const result = await fetchListings(year, make, model, trim, 'auction');
        console.log(result);
        setData(result);
      } catch (error) {
      ...
      <CardBody className="gap-4 w-full flex-row flex-wrap justify-around">
        {
          loading ? <Spinner /> :
            (data?.listings?.map((listing, index) => {
              return (
                <Card key={index} className="col-span-12 sm:col-span-4 max-h-[300px] max-w-[500px] min-w-[250px] flex-grow">
                  ...
                  <CardFooter className="absolute bg-black/75 bottom-0 border-t-1 border-zinc-100/50 z-10 justify-between">
                    <div className="flex flex-grow gap-2 items-center">
                      <div className="flex flex-col">
                        <p className="text-tiny text-white">Title: {listing.title_type || 'No Record'}</p>
